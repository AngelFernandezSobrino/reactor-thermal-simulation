...
class Simulador01DataShapes:
    nodes_wm_h = SimulationData.nodes_wm_h.shape
    nodes_ww_h = SimulationData.nodes_ww_h.shape
    nodes_ma_h = SimulationData.nodes_ma_h.shape
    nodes_mp_h = SimulationData.nodes_mp_h.shape

    nodes_w_t = SimulationData.nodes_w_t.shape
    nodes_m_t = SimulationData.nodes_m_t.shape

    p_t = SimulationData.p_t.shape

class Simulador01DataSizes:
    nodes_wm_h = SimulationData.nodes_wm_h.nbytes
    nodes_mp_h = SimulationData.nodes_mp_h.nbytes
    nodes_ww_h = SimulationData.nodes_ww_h.nbytes
    nodes_ma_h = SimulationData.nodes_ma_h.nbytes

    nodes_w_t = SimulationData.nodes_w_t.nbytes
    nodes_m_t = SimulationData.nodes_m_t.nbytes

    p_t = SimulationData.p_t.nbytes
...
def heat_wm(i):
    ...
    nodes_wm_h[i] = heat_rate_conduction_boundary(nodes_w_t[i], nodes_m_t[i], NodesParameters.Water.surface_metal, Reactor.HeatExchanger.water_metal_heat_transfer_coefficient)
...
while simulation_running and not end_simulation:
    ...
    for j in range(0, Parameters.Discretization.nodes):
        heats_calls.append(calculate_heat_rates_by_index.remote(SimulationData.nodes_w_t[j+1], SimulationData.nodes_w_t[j], SimulationData.nodes_m_t[j], SimulationData.p_t[0]))
    
    heat_results = ray.get(heats_calls)
    heat_results_list: list[list[float]] = list(zip(*heat_results))

    temp_calls = []
    for j in range(0, Parameters.Discretization.nodes):
        temp_calls.append(calculate_next_temperatures_by_index.remote(SimulationData.nodes_w_t[j+1], SimulationData.nodes_m_t[j], heat_results_list[0][j], heat_results_list[1][j], heat_results_list[2][j], heat_results_list[3][j], Parameters.Discretization.time_step))

    temperature_results = ray.get(temp_calls)
    temperature_results_list: list[list[float]] = list(zip(*temperature_results))
    ...
    if i % 100 == 0:
        ...
        for j in range(0, Parameters.Discretization.nodes):
            ...
            if m_delta_t > 0.5 or w_delta_t > 0.5 :
    ...
    # print(f'Actual time: {(i * Parameters.Discretization.time_step):.2f}s')
    # print(f'Actual time step: {Parameters.Discretization.time_step:.2f}s')
    # print(f'Simulation end time: {Parameters.time}s')

    data_time.append(i * Parameters.Discretization.time_step)
    
    # data_heat_wm.append(heat_results[node_to_measure][0])
    data_heat_ww.append(heat_results[node_to_measure][1])
    ...
    i += 1
    if i * Parameters.Discretization.time_step > Parameters.time:
